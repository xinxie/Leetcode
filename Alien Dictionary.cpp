...
private:
    ...
    bool dfs(unordered_set<char>& visited, unordered_set<char>& onpath, string& order, char key) {
        if(onpath.count(key)) return false;
        if(visited.count(key)) return true;
        visited.insert(key);
        onpath.insert(key);
        for(auto itr = graph[key].begin(); itr != graph[key].end(); ++itr) {
            if(dfs(visited, onpath, order, *itr) == false) { // check cyclic
                return false;
            }
        }
        onpath.erase(key);
...
public:
    string alienOrder(vector<string>& words) {
        ...
        for(int i = 1; i < words.size(); ++i) {
            ...
            // a flag avoding constructing cyclic graph.
            // Considering ["za", "zb", "ca", "cb"].  b->a, a->b, which is cyclic
            bool find = false;

            // make sure the second word cannot be the prefix of the first word.
            ...
            for(int j = 0; j < max(word1.length(), word2.length()); ++j) {
                ...
                if(j < word1.length() && j < word2.length() && word1[j] != word2[j] && !find) {
